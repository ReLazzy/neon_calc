...
export const KonvaLetterText: FC<KonvaLetterTextProps> = ({
  ...
  const { totalWidth, totalHeight, lineWidths } = useMemo(() => {
    ...
    splitted.forEach((line) => {
      ...
      for (const char of line) {
        ...
        if (glyph) {
          lw += glyph.width * (glyph.scale ?? 1) + letterSpacing;
        } else {
  ...
  const paths: React.ReactNode[] = useMemo(() => {
    // Начинаем с позиции по Y, чтобы текст располагался снизу
    let currentY = totalHeight - lines.length * lineHeight;
    ...
    lines.forEach((line, lineIndex) => {
      ...
      for (let i = 0; i < line.length; i++) {
        ...
        const { path, width, scale = 1 } = glyph;
        const xPos = offsetLineX + currentX;
        const yPos = currentY;

        currentX += width * scale + letterSpacing;
        ...
        items.push(
          <Path
            ...
            lineCap="round"
            lineJoin="round"
            shadowBlur={shadowBlur / (scaleFactor * (glyph.scale || 1))}
